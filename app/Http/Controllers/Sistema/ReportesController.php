...
";
        ...
        // ── Firma (única, con salto de página según configuración) ─────────────
        $informacionGeneral = InformacionGeneral::where('id', 1)->first();
    ...
    <div style='{$estiloSalto} padding-top:{$margenFirma}px;'>
        <table width='100%' style='font-size:13px; text-align:center; border-collapse:collapse;'>
            <tr>
                <td style='text-align:center;'>F._____________________________</td>
            </tr>
            <tr>
                <td style='text-align:center; padding-top:20px; font-weight:bold; font-size:14px;'>
        ...
        <tr style='background:#e9ecef;'>
            ...
            <td style='font-weight:bold; font-size:11px; padding:4px 6px; border:0.8px solid #bbb;'>\$ $montoFmt</td>
                ...
                if (!isset($resumenObjEsp[$info['objespec']])) {
                    $resumenObjEsp[$info['objespec']] = ['total' => 0];
                }
...
<table width='100%' style='border-collapse:collapse; font-family:Arial, sans-serif;'>
    <thead>
        ...
        <tr style='background:#6c757d;'>
            <td style='color:#fff; font-weight:bold; font-size:11px; padding:5px 8px; border:0.8px solid #888; width:50%;'>Cod. Presu.</td>
            <td style='color:#fff; font-weight:bold; font-size:11px; padding:5px 8px; border:0.8px solid #888; width:50%; text-align:right;'>Monto Total (\$)</td>
        </tr>
    </thead>
    <tbody>";
...
<table width='100%' style='border-collapse:collapse; font-family:Arial, sans-serif; margin-bottom:8px; border:0.8px solid #aaa;'>
    ...
    <tbody>";
        ...
        if ($tipo == 1) {
            ...
            // Cada fila de salidas_detalle ya es una línea de material entregado
            $query = SalidasDetalle::with([
                'entradaDetalle.material.unidadMedida',
                'entradaDetalle.material.objetoEspecifico',
            ]);
        ...
        <tr style='background:#e9ecef;'>
            ...
            if (!$sinFecha) $query->whereBetween('fecha', [$start, $end]);
            $arraySalidas = $query->orderBy('fecha', 'ASC')->orderBy('id', 'ASC')->get();

            // Agrupar las líneas que pertenecen a la misma salida:
            // misma fecha, departamento, tipo de salida, número de solicitud y descripción
            $grupos = [];
            foreach ($arraySalidas as $det) {
                $claveGrupo = implode('|', [
                    $det->fecha,
            ...
            foreach ($grupos as $filas) {
                ...
                $departamento = $primera->departamento->nombre ?? '—';
                $tipoSalida   = $primera->tipoSalida->nombre   ?? '—';
...
<table width='100%' style='border-collapse:collapse; font-family:Arial, sans-serif; margin-bottom:2px; border:0.8px solid #ccc;'>
    <tr>
        <td style='width:13%; border:0.8px solid #ccc; padding:5px 7px; font-size:11px; font-weight:bold; background:#f5f5f5;'>Fecha</td>
        <td style='width:20%; border:0.8px solid #ccc; padding:5px 7px; font-size:11px;'>$fechaFmt</td>
        <td style='width:15%; border:0.8px solid #ccc; padding:5px 7px; font-size:11px; font-weight:bold; background:#f5f5f5;'>Departamento</td>
        <td style='width:20%; border:0.8px solid #ccc; padding:5px 7px; font-size:11px;'>$departamento</td>
        <td style='width:12%; border:0.8px solid #ccc; padding:5px 7px; font-size:11px; font-weight:bold; background:#f5f5f5;'>Tipo Salida</td>
        <td style='width:20%; border:0.8px solid #ccc; padding:5px 7px; font-size:11px;'>$tipoSalida</td>
    </tr>
    <tr>
        <td style='border:0.8px solid #ccc; padding:5px 7px; font-size:11px; font-weight:bold; background:#f5f5f5;'>N° Solicitud</td>
        <td style='border:0.8px solid #ccc; padding:5px 7px; font-size:11px;'>$solicitud</td>
